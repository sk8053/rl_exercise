...
parser.add_argument("--n_prev_t", default = 5, type = int)
...
for episode in range(n_episode):
    ...
    #        trj_data = np.flip(trj_data, axis =0)
        ...
        for iter_ in np.arange(n_prev_t, iter_max):
            ...
            action[:bs_beam_idx1], action[bs_beam_idx1+1:8] = 0,0
            action[8:bs_beam_idx2+8], action[bs_beam_idx2 + 1+8:] = 0, 0
            ...
            #state_next = torch.cat([state[2:], torch.tensor([(new_bs_ind+1)/43,  (1+bs_beam_idx)/64,], dtype = torch.float32)])
            state_next = torch.cat([b_ind_window, bs_beam_ind_window])
                                                             #(1+uav_beam_idx)/16], dtype = torch.float32)])
