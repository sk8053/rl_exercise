...
class DDPG(nn.Module):
    ...
    def __init__(self,
        ...
        self.criteria =  nn.SmoothL1Loss()#nn.MSELoss() #
    ...
    def update(self, state, action, reward, state_next,  iter_=0):
        ...
        with torch.no_grad():
            ...
            #next_action_value = next_action.reshape(-1, 2, 6)#torch.vstack((next_action1,next_action2))
                        
            #min_idx = torch.argmin(next_action_value, 1)
            
            #for i in range(self.batch_size):    
            #    next_action_value[i][min_idx[i], range(6)] =0
                
            #next_action = next_action_value.reshape(-1, 12)
